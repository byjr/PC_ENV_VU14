...
#define show_rt_time(msg,show_func) ({\
	...
	strftime(buf,sizeof(TIME_LAY),TIME_FMT,timeinfo);\
...
#define getTmieStr(ts) ({\
	...
	if(timeStr){\
		struct tm* timeinfo;\
		timeinfo=localtime(&(ts).tv_sec);\
		strftime(timeStr,sizeof(TIME_LAY),TIME_FMT,timeinfo);\
